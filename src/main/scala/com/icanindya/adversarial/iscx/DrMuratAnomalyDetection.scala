...
object DrMuratAnomalyDetection {
  ...
  val kmModelDir = "D:/Data/ISCX_IDS/dr_murat/model/kmeans/%d"
  val kmModelClusterStatFile = kmModelDir + "/clsuters.txt"
  val kmAnomalyResultFilePath = "D:/Data/ISCX_IDS/dr_murat/anomaly_results/kmeans/k_%d_t_%.2f.txt"
  val pcFile = "D:/Data/ISCX_IDS/dr_murat/model/pc/%d"
...
//    for(k <- 80 to 100 by 5){
//      saveKMeansModel(sc, labeledPointTrainRdd, 25, k)
//    }
    
    for(k <- 80 to 100 by 5){
      for(c <- 1 to 10){
        val sqDistThres = Math.pow(c * 0.12, 2)
        saveKMeansAnomalyDetectionResult(sc, labeledPointTestRdd, 25, k, sqDistThres)
      }
    }
